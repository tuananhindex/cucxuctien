                  ...
                  <table class="table table-hover">
                    ...
                    @if(!empty($key))
                      @if(isset($data) && count($data) > 0)
                        @foreach($data as $val)
                        <?php 
                          $cats = DB::table('product_category')->get();
                          $ids = Helper::child_id($cats,$val->id);
                        ...
                        <tr>
                          ...
                          <td><a href="<?php echo route($e['route'].'.edit.get',$val->id) ?>">{{ ucfirst($val->name) }} ({{ $count }})</a></td>
...
<script type="text/javascript">
  $('input[name="price"]').change(function(){
    ...
    $.ajax({
      ...
      data : { _token : $('input[name="_token"]').val() , val : $(this).val() , id : id , table : 'product_category' },
      success : function(rs){
        //alert(rs);
      },
      error : function(err){
