				...
				<form method="post" enctype="multipart/form-data">
					...
					<div class="box-body">
						...
						<div class="form-group form-inline">
						  	<label style="margin-right: 20px">{{ ucwords(trans('common.event')) }}</label>
						   	<label class="radio-inline"><input type="radio" name="IsEvent" id="event-yes" value="1" @if($index->IsEvent) checked @endif>{{ ucwords(trans('common.yes')) }}</label>
							<label class="radio-inline"><input type="radio" name="IsEvent" id="event-no" value="0"  @if(!$index->IsEvent) checked @endif>{{ ucwords(trans('common.no')) }}</label>
							<div id="div-eventrange" class="form-inline" @if(!$index->IsEvent) style="display: none" @endif>
							<div class="form-group">
								<label style="margin-right: 20px">{{ ucwords(trans($e['lang'].'.daterange')) }}</label>
								<?php
									$startdate = str_replace('-', '/', $index->startdate);
									$enddate = str_replace('-', '/', $index->enddate); 
								?>
								<input type="text" class="form-control" name="daterange" value="@if($index->IsEvent){{ $startdate.' - '.$enddate }}@endif" style="width: 300px" />
							</div>
							</div>
