...
class DocumentController extends Controller
{
    ...
    public function add_get(){
        ...
        $cats = DB::table($this->e['table'].'_category')->join($this->e['table'].'_category_common',$this->e['table'].'_category'.'.fk_commonid','=',$this->e['table'].'_category_common.id')->where('nation',$this->__acc->nation)->where(['language' => $lang_id, 'IsDocument' => 1])->orderBy('order','desc')->orderBy('id','desc')->select($this->e['table'].'_category'.'.id','name','fk_parentid')->get();   
    ...
    public function add_post(Request $req){
        ...
        if($req->FilePath){
            $data['file'] = $req->FilePath;
        }   
        
        if($req->ngaybanhanh){
            $data['ngaybanhanh'] = AdminHelper::reformatDate($req->ngaybanhanh);
        }
        if($req->ngayhieuluc){
            $data['ngayhieuluc'] = AdminHelper::reformatDate($req->ngayhieuluc);
        }

        $data['IsEffect'] = $req->IsEffect;  
        $data['type'] = 'document';  
        $data['sokyhieu'] = $req->sokyhieu;
        ...
        $data_details['fk_postsId'] = $id;
        foreach($req->donvibanhanh as $key => $val){
            if($val != null || $req->nguoiky[$key] != null || $req->chucvu[$key] != null ){
                $data_details['donvibanhanh'] = $val;
                $data_details['nguoiky'] = $req->nguoiky[$key];
                $data_details['chucvu'] = $req->chucvu[$key];
                DB::table('document_details')->insert($data_details);
            }
        }
    ...
    public function edit_get($id,$lang = ''){
        ...
        return view($this->e['view'].'.edit',compact('index','posts_lang','cats','MultiLevelTreeData','posts','tags','languages','languages2','details'))->with(['e' => $this->e]);
    }
    ...
    public function edit_post(Request $req,$id,$lang = ''){
        ...
        $data_details['fk_postsId'] = $index->first()->id;
        foreach($req->donvibanhanh as $key => $val){
            if($val != null || $req->nguoiky[$key] != null || $req->chucvu[$key] != null ){
                $data_details['donvibanhanh'] = $val;
                $data_details['nguoiky'] = $req->nguoiky[$key];
    ...
    public function list_get($key = ''){
        ...
        $cats = DB::table($this->e['table'].'_category')->join($this->e['table'].'_category_common',$this->e['table'].'_category'.'.fk_commonid','=',$this->e['table'].'_category_common.id')->where(['nation' => $this->__acc->nation ,'IsDocument' => 1,'language' => $lang])->orderBy('order','desc')->orderBy('id','desc')->select($this->e['table'].'_category'.'.id','name','fk_parentid')->get();        
        if(Input::has('cat_id')){
        ...
        $data = DB::table($this->e['table'])->join($this->e['table'].'_common','fk_commonid','=',$this->e['table'].'_common.id')->where(['nation' => $this->__acc->nation , 'type' => 'document']);
    ...
    public function delete($id,$lang = ''){
     ...
     public function delete_details($id){
        $index = DB::table('document_details')->where('id',$id)->delete();
        return redirect()->back()->with(['alert' => AdminHelper::alert_admin('success','fa-check','Xóa dữ liệu thành công')]);
        //return redirect()->back();
    }
}
